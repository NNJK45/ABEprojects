...
<div class="news-area bg-white section pt-120 pb-70"><div class="container"><div class="row">
    <div class="col-xl-9 col-lg-8 col-12 mb-20"><article class="single-news-details">
        <img class="mb-4" src="{{ $actualite->image_url }}" alt="{{ $actualite->titre }}">
        ...
        @if($actualite->images->isNotEmpty())<div class="row mt-4">@foreach($actualite->images as $image)<a href="{{ $image->image_url }}" class="gallery-item image-popup col-md-4"><img src="{{ $image->image_url }}" alt="{{ $actualite->titre }}" loading="lazy"></a>@endforeach</div>@endif
    </article></div>
    <aside class="col-xl-3 col-lg-4 col-12"><div class="single-sidebar"><h4 class="title">Actualités récentes</h4><div class="recent-news">
        @forelse($recentes as $recente)<div class="recent-news-item"><a class="image" href="{{ route('news.details', $recente) }}"><img src="{{ $recente->image_url }}" alt="{{ $recente->titre }}" loading="lazy"></a><div class="content"><h5><a href="{{ route('news.details', $recente) }}">{{ $recente->titre }}</a></h5><p>{{ \Illuminate\Support\Str::limit($recente->contenu, 80) }}</p></div></div>@empty<p>Aucune autre actualité.</p>@endforelse
    </div></div></aside>
</div></div></div>
@endsection
